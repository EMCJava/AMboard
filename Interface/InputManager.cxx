...
bool
SButtonSet::ConsumeEvent( SButtonInputEvent Target )
{
    const auto RemoveIt = std::ranges::find( ButtonEvents, Target );
    if ( RemoveIt == ButtonEvents.end( ) ) return false;
    ButtonEvents.erase( RemoveIt );
    return true;
}

bool
SButtonSet::ConsumeHoldEvent( void* ContextObject, int GLFWKeyCode )
{
...
bool
SButtonSet::HasKeyEvent( ) const noexcept
{
    return !ButtonEvents.empty( );
}

bool
SButtonSet::IsKeyDown( int GLFWKeyCode ) const noexcept
{
    return KeyStages[ GLFWKeyCode ].IsDown;
}

bool
SButtonSet::IsKeyDoubleClick( int GLFWKeyCode ) const noexcept
{
...
void
CInputManager::RegisterKeyInput( int key, [[maybe_unused]] int, int action, int mods )
{
...
void
CInputManager::RegisterMouseInput( int key, int action, int mods )
{
...
void
CInputManager::RegisterCharInput( uint32_t CodePoint )
{
...
bool
CInputManager::HasCharEvent( ) const noexcept
{
    return !CharEvents.empty( );
}

bool
CInputManager::IsCharacterDown( char CharacterAToZ ) const noexcept
{
...
void
CInputManager::SetLockMouse( bool Lock ) noexcept
{
    if ( ( MouseLocked = Lock ) )
    {
...
bool
CInputManager::IsMouseLocked( ) const noexcept
{
    return MouseLocked;
}

void
CInputManager::ResetCursorPosition( ) noexcept
{
    double XPos, YPos;
...
void
CInputManager::RegisterCursorPosition( int cursorX, int cursorY )
{
...
void
CInputManager::RegisterCursorScroll( int delta )
{
...
int
CInputManager::GetDeltaScroll( ) const noexcept
{
